...
int NSP_TDYTH_4G_accessAuth_User_Identity_Request_Forward(int iSockfd, char *pcRecvBuf, char *pcSendBuf)
{
    ...
    if ((iRet = NSP_TDYTH_4G_ANET_ipv4_send(iSockfd, pcSendBuf,D_NSP_TDYTH_NAAAS_4G_BUFF_LEN,
                                            stAuBodyFirstMMEToENB->aucDstId, stAuBodyFirstMMEToENB->aucDstPort, 0)) < 0)
    {
...
*/
    ...
    //todo KENB为保存的
    printf("开始生成密钥！\n");
    if ((iRet = NSP_TDYTH_4G_gen_Kenb(aucKenb, gsSelfENB.uiPCI, gsSelfENB.uiEARFCN, stSendBody.aucKenb)) != 0) {
...
int NSP_TDYTH_4G_S1_HandOver_Second_SENBSendToMME(char *pcRecvBuf, char *pcSendBuf)
{
    ...
    stSendBody.TimeStamp = NSP_TDYTH_4G_get_time_now();
    stSendBody.aucIdNumber = 0x1;
    memcpy(stSendBody.aucDstId, pstRecvBody->aucDstId, D_NSP_TDYTH_NAAAS_4G_ID_LEN);
    memcpy(stSendBody.aucSrcId, pstRecvBody->aucSrcId, D_NSP_TDYTH_NAAAS_4G_ID_LEN);
    stSendBody.aucDstPort = pstRecvBody->aucDstPort;
    stSendBody.aucSrcPort = pstRecvBody->aucSrcPort;
    stSendBody.iDataLen = 0;
...
int NSP_TDYTH_4G_TAU_Second_ENBToMME(char *pcRecvBuf, char *pcSendBuf, int iSockfd)
{
    ...
    memcpy(stTauBodySecondENBToMME.aucDstId, D_NSP_TDYTH_NAAAS_MME2_IP, D_NSP_TDYTH_NAAAS_4G_ID_LEN);
    stTauBodySecondENBToMME.uiDstPort = D_NSP_TDYTH_NAAAS_MME2_PORT;
    memcpy(stTauBodySecondENBToMME.aucSrcId, D_NSP_TDYTH_NAAAS_ENB2_IP, D_NSP_TDYTH_NAAAS_4G_ID_LEN);
    stTauBodySecondENBToMME.uiSrcPort = D_NSP_TDYTH_NAAAS_ENB2_PORT;
    ...
    memcpy(stTauBodySecondENBToMME.aucNewTAI, D_NSP_TDYTH_NAAAS_4G_ENB2_TAI, D_NSP_TDYTH_NAAAS_4G_TAI_LEN);
...
void * NSP_TDYTH_4G_ListenAccept_eNB_Function(int* iSockfd)
{
    ...
    while(1)
    {
        ...
        iRet = recvfrom(*iSockfd, cRecvBuf, D_NSP_TDYTH_NAAAS_4G_UE_BUFF_LEN, MSG_DONTWAIT,
                        (struct sockaddr *) &ENB2Sockaddr, &iSinLen);
        if (iRet < 0)
        {
            return NULL;
        }
                    ...
                    switch (pstHeader->ucStep)   // 判断步骤
                    {
                        ...
                        case 0x07:
                            ...
                            if ((iRet = sendto(*iSockfd, cSendBuf, D_NSP_TDYTH_NAAAS_4G_ENB_BUFF_LEN,
                                               0, (struct sockaddr *) &MMESockaddr, sizeof(MMESockaddr))) < 0)
                            {
                                printf("%s,[line=%d],errno=%d:%s\n", "UE:UDP ipv4 socket err", __LINE__, errno,
                                       strerror(errno));
                                return NULL;
...
{
    // 构造广播包内容（以下数据仅针对ENB2，之后可以修改为动态数据）
    //printf("开始构造广播包内容\n");
    St_NSP_TDYTH_NAAAS_4G_Header stHeader;
    ...
    gsSelfENB.uiENBID = D_NSP_TDYTH_NAAAS_4G_ENB2_ENBID;
    gsSelfENB.uiPCI = D_NSP_TDYTH_NAAAS_4G_ENB2_PCI;
    gsSelfENB.uiEARFCN = D_NSP_TDYTH_NAAAS_4G_ENB2_EARFCN;
    gsSelfENB.iRSRP = D_NSP_TDYTH_NAAAS_4G_ENB2_RSRP;
    gsSelfENB.iCellRP = D_NSP_TDYTH_NAAAS_4G_ENB2_CellRP;
    gsSelfENB.iQrxlevmin = D_NSP_TDYTH_NAAAS_4G_ENB2_Qrxlevmin;
    memcpy(gsSelfENB.aucTAI, D_NSP_TDYTH_NAAAS_4G_ENB2_TAI, D_NSP_TDYTH_NAAAS_4G_TAI_LEN);

    memcpy(gsSelfENB.aucENBIp, D_NSP_TDYTH_NAAAS_ENB2_IP, D_NSP_TDYTH_NAAAS_4G_ID_LEN);
    gsSelfENB.uiENBPort = D_NSP_TDYTH_NAAAS_ENB2_PORT;
...
int NSP_TDYTH_4G_Broadcast_eNB_Function()
{
    ...
    ENBSockaddr.sin_port = htons(D_NSP_TDYTH_NAAAS_ENB2_PORT);
